...
def handler(cursor: Optional[str], limit: int, requester_did: str) -> dict:
    ...
    posts = db.session.scalars( sa.select(Post).join(UserFollows, Post.did == UserFollows.follows_did).where(where_stmt).order_by(Post.indexed_at.desc()).order_by(Post.cid.desc()).limit(limit) ).all()
